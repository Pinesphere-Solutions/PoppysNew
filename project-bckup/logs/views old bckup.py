...
class MachineReport(APIView):    
    ...
    def get(self, request, *args, **kwargs):
        ...
        # --- Always fetch all logs unless a filter is provided ---
        logs_qs = MachineLog.objects.all()
        if machine_id_filter:
            logs_qs = logs_qs.filter(MACHINE_ID=machine_id_filter)
        if from_str and to_str:
            logs_qs = logs_qs.filter(DATE__gte=from_str, DATE__lte=to_str)
        elif date_str:
            logs_qs = logs_qs.filter(DATE=date_str)

        # Get all unique dates from the filtered logs
        report_dates = logs_qs.values_list('DATE', flat=True).distinct()
        ...
        if from_str and to_str:
            # Use raw string comparison for filtering
        ...
        # Get all logs to process (filtered or all)
        logs = list(logs_qs)

        # Group logs by (date, machine_id)
        from collections import defaultdict
        logs_by_date_machine = defaultdict(list)
        for log in logs:
            logs_by_date_machine[(log.DATE, log.MACHINE_ID)].append(log)
        ...
        for idx, ((report_date, machine_id), group_logs) in enumerate(logs_by_date_machine.items(), 1):
            # Now, use group_logs as the logs for this (date, machine) group
            # Copy your per-date logic here, replacing 'logs = ...' with 'group_logs'
            # For example:
            # logs = group_logs
            # ...rest of your per-date logic, using 'logs' as the list...

            # You can copy-paste your per-date logic here, just use 'logs = group_logs' at the start of the loop
            ...
            logs = group_logs
            logger.info(f"Initial logs count for date {report_date}: {len(logs)}")
            if machine_id_filter:
                logs = [log for log in logs if log.MACHINE_ID == int(machine_id_filter)]
                logger.info(f"Filtered logs by MACHINE_ID={machine_id_filter}: {len(logs)}")
            if date_str:  # Only filter by date if date parameter is provided
                logs = [log for log in logs if str(log.DATE) == str(report_date)]
                logger.info(f"Filtered logs by DATE={report_date}: {len(logs)}")

            # --- Exclude logs that are entirely within break times ---
            # (keep your break exclusion logic here, unchanged)
            logs = list(logs)  # convert to list if needed

            # The rest of your per-date logic goes here, unchanged
            # (all the calculations, exclusions, and summary building)
            # ...existing code for per-date logic...

            # The following block is your existing per-date logic, unchanged:
            for log in logs:
                try:

...
export default function Operator() {
  ...
  // Dynamically fetch operator IDs for dropdown
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/poppys-operator-logs/")
      .then(res => {
        const ids = (res.data.summary || []).map(row => row["Operator ID"]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // ...existing code...
      const res = await axios.get("http://localhost:8000/api/operator-report/");
      // ...existing code...

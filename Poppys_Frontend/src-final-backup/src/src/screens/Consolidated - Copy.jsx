...
export default function Consolidated() {
    ...
    const fetchOptions = async () => {
      try {
        ...
        const operatorRes = await axios.get(
          "https://oceanatlantic.pinesphere.co.in/api/operator-report/"
        );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (selectedOperatorIds.length > 0)
        params.operator_ids = selectedOperatorIds.join(",");
      ...
      if (selectedFilter === "operator" || pendingFilter === "operator") {
        ...
        const mappedRows = backendRows.map((row, idx) => {
          ...
          return {
            ...
            maintenanceDecimal:
              convertTimeToHours(row["Maintenance Hours"]) || 0,
        ...
        console.log(
          "Fetching consolidated data for 'all' filter with enhanced operator mapping..."
        );
        ...
        try {
          ...
          const operatorRawRes = await axios.get(
            "https://oceanatlantic.pinesphere.co.in/api/operator-report/raw/",
            { params: {} }
          ); // No params = get all
          operatorRawRows = operatorRawRes.data.raw_data || [];

          console.log(
            `✅ Fetched ${operatorRows.length} operator summary records and ${operatorRawRows.length} operator raw records`
          );
        } catch (err) {
        ...
        try {
          ...
          console.log(
            `✅ Fetched ${machineRawRows.length} machine raw records`
          );
        } catch (e) {
        ...
        operatorRows.forEach((row) => {
          ...
          if (opId && opName) {
            operatorIdToNameMap.set(
              opId.toString().trim(),
              opName.toString().trim()
            );
            operatorNameToIdMap.set(
              opName.toString().trim().toLowerCase(),
              opId.toString().trim()
            );
          }
        });
        ...
        const mappedRows = machineRows.map((row, idx) => {
          ...
          if ((!operatorId || !operatorName) && machineId && date) {
            ...
            if (rawData) {
              ...
              operatorName =
                operatorName ||
                rawData["Operator Name"] ||
                rawData.operatorName ||
                "";
              lineNumber =
                lineNumber ||
                rawData["Line Number"] ||
          ...
          if (operatorId && !operatorName) {
            operatorName =
              operatorIdToNameMap.get(operatorId.toString().trim()) || "";
          }

          if (operatorName && !operatorId) {
            operatorId =
              operatorNameToIdMap.get(
                operatorName.toString().trim().toLowerCase()
              ) || "";
          }
          ...
          if ((!operatorId || !operatorName) && machineId && date) {
            ...
            if (matchingOperatorRecord) {
              operatorId =
                operatorId ||
                matchingOperatorRecord["Operator ID"] ||
                matchingOperatorRecord["operator_id"] ||
                "";
              operatorName =
                operatorName ||
                matchingOperatorRecord["Operator Name"] ||
                matchingOperatorRecord["operator_name"] ||
                "";
            }
          }
          ...
          const finalOperatorName =
            operatorName || (operatorId ? `Operator-${operatorId}` : "Unknown");
        ...
        console.log(
          "✅ Enhanced mapping completed for 'all' filter with comprehensive operator data:",
          mappedRows
        );
        setData(mappedRows);
      }
  ...
  const applyFilters = (dataToFilter) => {
    ...
    if (machineIds.length > 0) {
      filtered = filtered.filter((row) => {
        ...
        return machineIds.some(
          (selectedId) => selectedId.toString() === rowMachineId
        );
      });
    }
    ...
    if (selectedOperatorIds.length > 0) {
      filtered = filtered.filter((row) => {
        ...
        return selectedOperatorIds.some(
          (selectedId) => selectedId.toString() === rowOperatorId
        );
      });
    }
    ...
    if (lineIds.length > 0) {
      filtered = filtered.filter((row) => {
        ...
        return lineIds.some(
          (selectedId) => selectedId.toString() === rowLineId
        );
      });
    }
  ...
  const totalSewing = filtered.reduce(
    (sum, row) => sum + (parseFloat(row.sewingDecimal) || 0),
    0
  );
  const totalIdle = filtered.reduce(
    (sum, row) => sum + (parseFloat(row.idleDecimal) || 0),
    0
  );
  const totalRework = filtered.reduce(
    (sum, row) => sum + (parseFloat(row.reworkDecimal) || 0),
    0
  );
  const totalNoFeeding = filtered.reduce(
    (sum, row) => sum + (parseFloat(row.noFeedingDecimal) || 0),
    0
  );
  const totalMeeting = filtered.reduce(
    (sum, row) => sum + (parseFloat(row.meetingDecimal) || 0),
    0
  );
  const totalMaintenance = filtered.reduce(
    (sum, row) => sum + (parseFloat(row.maintenanceDecimal) || 0),
    0
  );
  const totalNeedleBreak = filtered.reduce(
    (sum, row) => sum + (parseFloat(row.needleBreakDecimal) || 0),
    0
  );
  ...
  const fetchRawData = async () => {
    ...
    try {
      ...
      if (selectedFilter === "operator" || pendingFilter === "operator") {
        ...
        rawDataResponse = await axios.get(
          "https://oceanatlantic.pinesphere.co.in/api/operator-report/raw/",
          { params: {} }
        );
      } else if (selectedFilter === "line" || pendingFilter === "line") {
      ...
      const backendRawRows =
        rawDataResponse.data.raw_data || rawDataResponse.data || [];
      ...
      const mappedRawRows = backendRawRows.map((row, idx) => ({
        ...
        modeDescription:
          ...
          getModeDescription(row["Mode"] || row["mode"]),
  ...
  const applyRawDataFilters = (dataToFilter) => {
    ...
    if (
      (selectedFilter === "operator" || pendingFilter === "operator") &&
      selectedOperatorIds.length > 0
    ) {
      filtered = filtered.filter(
        (row) =>
          row.operatorId &&
          selectedOperatorIds.some(
            (selectedId) => selectedId.toString() === row.operatorId.toString()
          )
      );
    }
    ...
    if (
      (selectedFilter === "machine" || pendingFilter === "machine") &&
      machineIds.length > 0
    ) {
      filtered = filtered.filter(
        (row) =>
          row.machineId &&
          machineIds.some(
            (selectedId) => selectedId.toString() === row.machineId.toString()
          )
      );
    }
    ...
    if (
      (selectedFilter === "line" || pendingFilter === "line") &&
      lineIds.length > 0
    ) {
      filtered = filtered.filter(
        (row) =>
          row.lineNumber &&
          lineIds.some(
            (selectedId) => selectedId.toString() === row.lineNumber.toString()
          )
      );
    }
  ...
  const fetchOperatorMapping = async () => {
    try {
      ...
      const response = await axios.get(
        "https://oceanatlantic.pinesphere.co.in/api/operators/"
      );
  ...
  const handleGenerate = async () => {
    ...
    if (!from || !to) {
      alert(
        "Please select both 'From' and 'To' dates before generating the report."
      );
      return;
    }
  ...
  const rawPageCount = Math.max(
    1,
    Math.ceil(filteredRawData.length / rowsPerPage)
  );
  const rawPage = 1; // You might want to add a separate rawPage state
  const rawPaginated = filteredRawData.slice(
  ...
  const handleCSV = () => {
    ...
    a.download = showRawData
      ? "consolidated_raw_data_filtered.csv"
  ...
  const handleHTML = () => {
    ...
    let htmlContent = `
      ...
      <body>
        <div class="header">
          <h1>${
            showRawData ? "Consolidated Raw Data Report" : "Consolidated Report"
          }</h1>
          <p><strong>Generated on:</strong> ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}</p>
          <p><strong>Total Records:</strong> ${dataToExport.length}</p>
          ${
            from || to
              ? `<p><strong>Date Range:</strong> ${from || "Start"} to ${
                  to || "End"
                }</p>`
              : ""
          }
          ${
            machineIds.length > 0
              ? `<p><strong>Machine IDs:</strong> ${machineIds.join(", ")}</p>`
              : ""
          }
          ${
            selectedOperatorIds.length > 0
              ? `<p><strong>Operator IDs:</strong> ${selectedOperatorIds.join(
                  ", "
                )}</p>`
              : ""
          }
          ${
            lineIds.length > 0
              ? `<p><strong>Line IDs:</strong> ${lineIds.join(", ")}</p>`
              : ""
          }
        </div>
    ...
    htmlContent += `
        ...
        <div class="footer">
          ...
          <p>Data includes ${
            !showRawData ? "summarized" : "detailed raw"
          } information for the selected period</p>
        </div>
      </body>
      </html>
    `;
    ...
    link.href = url;
    link.download = `${
      showRawData ? "consolidated_raw_data" : "consolidated_report"
          ...
          >
            ...
            {lineIds.length > 0 && (
              <span style={{ marginLeft: "8px" }}>
                ...
                {lineIds.length === 1
                  ? lineIds[0]
                  : `${lineIds.length} selected`}
              </span>
            )}
            <span style={{ marginLeft: "8px" }}>
              ({showRawData ? filteredRawData.length : filtered.length} of{" "}
              {showRawData ? rawData.length : data.length} records)
            </span>
            {(showRawData ? filteredRawData.length : filtered.length) === 0 && (
      ...
      {showInitialState ? (
        ...
        <div className="machine-table-card">
          ...
          >
            ...
            >
              Select your filters and click <strong>Generate</strong> to view
            ...
            >
              💡 Tip: Use filters to analyze specific machines, operators, or
              time periods
            </div>
          </div>
        </div>
        ...
        <div className="machine-table-card">
          ...
          {!showRawData && filtered.length > 0 && (
            <>
              {/* Tiles Row */}
              <div
                className="machine-tiles-row machine-tiles-row-full"
              ...
              {pieData.length > 0 && (
                ...
                >
                  ...
                  <div className="machine-pie-info">
                    ...
                    >
                      Hours Breakdown ({filtered.length} records:{" "}
                    ...
                    <div style={{ fontSize: "14px", lineHeight: "1.6" }}>
                      ...
                      <div>
                        {formatHoursToTime(totalNoFeeding)} : No Feeding Hours
                      </div>
                      <div>
                        {formatHoursToTime(totalMeeting)} : Meeting Hours
                      </div>
                      <div>
                        {formatHoursToTime(totalMaintenance)} : Maintenance
                        Hours
                      </div>
                      <div>
                        {formatHoursToTime(totalNeedleBreak)} : Needle Break
                        Hours
                      </div>
                    </div>
                  </div>
